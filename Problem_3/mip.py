...
class Verifier:
    ...
    def create_inputs(self, x_test, perturbation):
        ...
        for i in range(len(x_test)):
            # Append to x_vars - a list of variable having lower and upper bounds
            self.x_vars.append(
                self.gurobi_model.addVar(
                    lb=x_test[i] - perturbation,
                    ub=x_test[i] + perturbation,
    ...
    def add_linear_layer(self, input_vars, weight, bias):
        ...
        self.gurobi_model.update()
        # Add constraints for variable z representing the output of the layer before activation i.e. wx + b
        for i in range(output_size):
            self.gurobi_model.addConstr(
                z[i]
                == gurobipy.quicksum(
                    weight[i][j] * input_vars[j] for j in range(input_size)
                )
                + bias[i]
            )
        self.gurobi_model.update()
        print(
            f"Created linear layer with input shape {input_size} output shape {output_size}"
        )
        return z.values()
    ...
    def add_relu_layer(self, z, lower_bounds, upper_bounds):
        ...
        # Model ReLU activation as piecewise linear constraints in MIP.
        ...
        print(
            f"Created relu layer with {n_unstable} unstable neurons out of {n_inputs} total neurons"
        )
    ...
    def solve_objectives(self, objectives, direction="minimization"):
        ...
        for i in range(len(objectives)):
            ...
            # Add Gurobi objectives to the model
            ...
            # Check if gurobi model is at optimal status
            assert self.gurobi_model.status == gurobipy.GRB.OPTIMAL
    ...
    def get_verification_objectives(self, y_vars, groundtruth_label):
        ...
        objectives = []
        target_labels = []
        # Formulate the verification objectives for the MIP model.
        ...
        for i in range(len(y_vars)):
            ...
            # append objective that needs to be minimized, think about the original label and the one after perturbation
...
def load_model_and_data(data):
    ...
    model = SimpleNN(
        input_size=28 * 28, hidden_size_1=128, hidden_size_2=64, output_size=10
    )
...
def verify(model, input_image, groundtruth_label, perturbation):
    ...
    for i in range(num_layers):
        ...
        if i != num_layers - 1:
            ...
            print(
                f"Calculating intermediate layer bounds for {len(variables)} neurons..."
            )
    ...
    for i, r in enumerate(target_labels):
        ...
        print(
            f"Minimum y_{groundtruth_label} - y{r} = {optimal_objs[i]:13.8g}; calculated margin = {margin:13.8g}"
        )
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    result = verify(model, x_test, groundtruth_label, perturbation=0.03)
