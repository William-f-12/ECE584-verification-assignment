...
from hardTanh_alphaCrownVer import BoundHardTanh
...
class BoundedSequential(nn.Sequential):
    ...
    def compute_bounds(
        ...
        """
        if optimize:
            ub, lb = self.alpha_crown(x_U=x_U, x_L=x_L, upper=upper, lower=lower)
        else:
    ...
    def alpha_crown(self, x_U=None, x_L=None, upper=True, lower=True, optim_steps=100):
        r"""alpha-CROWN bound propagation with optimized alpha parameters.
    ...
    def boundpropogate_from_layer(
        ...
        for i, module in enumerate(reversed(modules)):
            ...
            upper_A, upper_b, lower_A, lower_b = module.boundpropogate(
                upper_A, lower_A, start_node, alpha=alpha
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--optimize",
        action="store_true",
        help="Use alpha-CROWN optimization",
    )
    parser.add_argument(
    ...
    ub, lb = boundedmodel.compute_bounds(
        x_U=x_u, x_L=x_l, upper=True, lower=True, optimize=args.optimize
    )
